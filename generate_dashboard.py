...
try:
    ...
    if data and len(data) > 0:
        ...
        # Apply cleaning to all headers for DataFrame creation
        # This is the crucial step to ensure initial DataFrame columns are clean
        df_columns_for_creation = [clean_header_string(h) for h in raw_headers]

        # Create DataFrame directly with cleaned headers
        df = pd.DataFrame(data_rows, columns=df_columns_for_creation)

        # --- DEBUG PRINT: DataFrame head and columns (after initial cleaning) ---
        print("--- DataFrame Head (after initial cleaning): ---")
        print(df.head())
        print("--- DataFrame Columns (after initial cleaning): ---")
        print(df.columns.tolist())
        print("-----------------------------------------")

        # --- Data Aggregation for Dashboard Statistics ---
        # Identify all Seksu and Idade columns based on their *cleaned* names
        # They will appear as 'Seksu' and 'Idade' multiple times
        sek_cols_for_melt = [col for col in df.columns if col == 'Seksu']
        idade_cols_for_melt = [col for col in df.columns if col == 'Idade']

        # Get unique values for other key columns from the original (cleaned) DataFrame
        # This is important for the overall counts
        munisipiu_unique = df['Munisipiu'].unique() if 'Munisipiu' in df.columns else []
        nivel_eskola_unique = df['Nivel Eskola'].unique() if 'Nivel Eskola' in df.columns else []
        disciplina_unique = df['Dixiplina'].unique() if 'Dixiplina' in df.columns else []
        titulu_topiku_unique = df['Titulu/Tópiku Atividade'].unique() if 'Titulu/Tópiku Atividade' in df.columns else []


        # Create a list of DataFrames for each 'Kanorin' entry
        # This handles the multiple Seksu/Idade columns by treating each Kanorin as a separate observation
        processed_records = []
        for i in range(1, 4): # Assuming Kanorin 1, 2, 3
            kanorin_seksu_col = f'Seksu_{i}' if f'Seksu_{i}' in df.columns else 'Seksu' # Fallback to 'Seksu' if no suffix
            kanorin_idade_col = f'Idade_{i}' if f'Idade_{i}' in df.columns else 'Idade' # Fallback to 'Idade' if no suffix

            # Check if these specific columns exist in the DataFrame
            if kanorin_seksu_col in df.columns and kanorin_idade_col in df.columns:
                temp_df = df[[
                    'Timestamp', 'Email Address', 'Munisipiu', 'Nivel Eskola', 'Naran Eskola',
                    'Dixiplina', 'Títulu/Tópiku Atividade', kanorin_seksu_col, kanorin_idade_col
                ]].copy()
                temp_df.rename(columns={
                    kanorin_seksu_col: 'Seksu',
                    kanorin_idade_col: 'Idade',
                    'Títulu/Tópiku Atividade': 'Titulu/Tópiku' # Standardize for dashboard
                }, inplace=True)
                processed_records.append(temp_df)
            elif 'Seksu' in df.columns and 'Idade' in df.columns and i == 1: # Fallback for single Seksu/Idade if no Kanorin suffix
                 temp_df = df[[
                    'Timestamp', 'Email Address', 'Munisipiu', 'Nivel Eskola', 'Naran Eskola',
                    'Dixiplina', 'Títulu/Tópiku Atividade', 'Seksu', 'Idade'
                ]].copy()
                 temp_df.rename(columns={
                    'Títulu/Tópiku Atividade': 'Titulu/Tópiku' # Standardize for dashboard
                }, inplace=True)
        ...
        if processed_records:
            # Concatenate all processed records into one DataFrame
            ...
            # Detailed Table Data - Use the original df (with cleaned initial headers) for this
            # Ensure the columns for the detailed table match the HTML structure
            # We need to explicitly select the columns that are expected in the HTML table
            # and map them to the cleaned names.
            
            # Define the exact column names expected in the HTML detailed table headers
            # These are the *cleaned* names from the raw data, before any melting.
            html_table_cols = [
                'Munisipiu', 'Seksu', 'Idade', 'Dixiplina', 'Nivel Eskola',
                'Naran Eskola', 'Titulu/Tópiku Atividade', 'Timestamp'
            ]
            
            # Create a list of dictionaries for the detailed table data
            # Iterate through the original DataFrame (df) and pick the relevant columns
            detailed_data_for_html = []
            for index, row in df.iterrows():
                row_dict = {}
                # Map the cleaned column names from df to the expected HTML table keys
                # This requires careful handling of the multiple Seksu/Idade columns if they are to be displayed.
                # For simplicity, let's just pick the first Seksu and Idade for display in the detailed table
                # if multiple exist. Or better, concatenate them.
                
                # Let's collect all Seksu and Idade values for a given row and display them as a comma-separated string
                all_seksu_in_row = [row[col] for col in df.columns if col.startswith('Seksu') and pd.notna(row[col]) and row[col] != '']
                all_idade_in_row = [row[col] for col in df.columns if col.startswith('Idade') and pd.notna(row[col]) and row[col] != '']
                ...
                row_dict['Titulu/Tópiku'] = row.get('Títulu/Tópiku Atividade', 'N/A') # Use the actual cleaned name
            ...
            for row in dashboard_data["detailedTableData"]:
                row['id'] = str(row.get('id', 'N/A')) # Ensure 'id' exists
...
```Okay, you've provided the latest debug output, and it's clear we're still battling the column naming issue. My apologies for the persistence of this problem.

The output:
`--- DataFrame Columns (after initial cleaning): ---`
`['Timestamp', 'Email Address', "Munisípiu\n* Eskola lokaliza iha munisípiu ne'ebé. Ez. Aileu.", ... 'Seksu', 'Idade', ...]`

This confirms that even after introducing `df_columns_for_creation` and `clean_header_string`, the DataFrame is still being created with the *original, uncleaned, verbose column names*. This is highly unusual for `pd.DataFrame(data_rows, columns=df_columns_for_creation)`. It implies that `data_rows` itself might be carrying some implicit header information or that the `columns` argument is being overridden.

The `Grouper for 'Seksu' not 1-dimensional` error is a direct consequence of this: if multiple columns are named `Seksu` (after the cleaning logic, which is *not* being applied to the DataFrame's actual headers), then `value_counts()` will fail.

**The core problem is that the DataFrame's column names are not being set correctly to the *cleaned* versions.**

Let's try a more direct and robust approach to ensure the column names are cleaned immediately after the DataFrame is created. We will create the DataFrame first with the raw headers, and then explicitly rename its columns using our cleaning function.
